...
type WorkflowQuery struct {
	...
	withWorkflowNodes *WorkflowNodeQuery
	withTasks         *TaskQuery
...
// QueryWorkflowNodes chains the current query on the "workflow_nodes" edge.
func (wq *WorkflowQuery) QueryWorkflowNodes() *WorkflowNodeQuery {
	query := (&WorkflowNodeClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) QueryTasks() *TaskQuery {
...
func (wq *WorkflowQuery) Clone() *WorkflowQuery {
	...
	return &WorkflowQuery{
		...
		withWorkflowNodes: wq.withWorkflowNodes.Clone(),
...
// WithWorkflowNodes tells the query-builder to eager-load the nodes that are connected to
// the "workflow_nodes" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WorkflowQuery) WithWorkflowNodes(opts ...func(*WorkflowNodeQuery)) *WorkflowQuery {
	query := (&WorkflowNodeClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withWorkflowNodes = query
	return wq
}

// WithTasks tells the query-builder to eager-load the nodes that are connected to
// the "tasks" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WorkflowQuery) WithTasks(opts ...func(*TaskQuery)) *WorkflowQuery {
	query := (&TaskClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (wq *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := wq.withWorkflowNodes; query != nil {
		if err := wq.loadWorkflowNodes(ctx, query, nodes,
			func(n *Workflow) { n.Edges.WorkflowNodes = []*WorkflowNode{} },
			func(n *Workflow, e *WorkflowNode) { n.Edges.WorkflowNodes = append(n.Edges.WorkflowNodes, e) }); err != nil {
			return nil, err
		}
	}
...
func (wq *WorkflowQuery) loadWorkflowNodes(ctx context.Context, query *WorkflowNodeQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *WorkflowNode)) error {
	...
	query.Where(predicate.WorkflowNode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.WorkflowNodesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.workflow_workflow_nodes
		if fk == nil {
			return fmt.Errorf(`foreign-key "workflow_workflow_nodes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workflow_workflow_nodes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (wq *WorkflowQuery) loadTasks(ctx context.Context, query *TaskQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *Task)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Workflow)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.workflow_tasks
		if fk == nil {
			return fmt.Errorf(`foreign-key "workflow_tasks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workflow_tasks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
