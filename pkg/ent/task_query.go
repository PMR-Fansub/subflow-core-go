...
func (tq *TaskQuery) QueryTaskRecords() *TaskRecordQuery {
	query := (&TaskRecordClient{config: tq.config}).Query()
...
func (tq *TaskQuery) QueryWorkflow() *WorkflowQuery {
...
func (tq *TaskQuery) QueryTeam() *TeamQuery {
...
func (tq *TaskQuery) WithTaskRecords(opts ...func(*TaskRecordQuery)) *TaskQuery {
	query := (&TaskRecordClient{config: tq.config}).Query()
...
func (tq *TaskQuery) WithWorkflow(opts ...func(*WorkflowQuery)) *TaskQuery {
	query := (&WorkflowClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withWorkflow = query
	return tq
}

// WithTeam tells the query-builder to eager-load the nodes that are connected to
// the "team" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithTeam(opts ...func(*TeamQuery)) *TaskQuery {
	query := (&TeamClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if tq.withWorkflow != nil || tq.withTeam != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withTaskRecords; query != nil {
	...
	if query := tq.withWorkflow; query != nil {
		if err := tq.loadWorkflow(ctx, query, nodes, nil,
			func(n *Task, e *Workflow) { n.Edges.Workflow = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTeam; query != nil {
		if err := tq.loadTeam(ctx, query, nodes, nil,
			func(n *Task, e *Team) { n.Edges.Team = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadTaskRecords(ctx context.Context, query *TaskRecordQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskRecord)) error {
	...
	query.Where(predicate.TaskRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.TaskRecordsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.task_task_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_task_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (tq *TaskQuery) loadWorkflow(ctx context.Context, query *WorkflowQuery, nodes []*Task, init func(*Task), assign func(*Task, *Workflow)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i].workflow_tasks == nil {
			continue
		}
		fk := *nodes[i].workflow_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "workflow_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Task, init func(*Task), assign func(*Task, *Team)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Task)
	for i := range nodes {
		if nodes[i].team_tasks == nil {
			continue
		}
		fk := *nodes[i].team_tasks
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "team_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
