...
type TaskRecordQuery struct {
	...
	withUser         *UserQuery
	withTask         *TaskQuery
	withWorkflowNode *WorkflowNodeQuery
	withFKs          bool
...
func (trq *TaskRecordQuery) QueryUser() *UserQuery {
...
// QueryWorkflowNode chains the current query on the "workflow_node" edge.
func (trq *TaskRecordQuery) QueryWorkflowNode() *WorkflowNodeQuery {
	query := (&WorkflowNodeClient{config: trq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := trq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (trq *TaskRecordQuery) Clone() *TaskRecordQuery {
	...
	return &TaskRecordQuery{
		...
		withUser:         trq.withUser.Clone(),
		withTask:         trq.withTask.Clone(),
		withWorkflowNode: trq.withWorkflowNode.Clone(),
...
func (trq *TaskRecordQuery) WithUser(opts ...func(*UserQuery)) *TaskRecordQuery {
	query := (&UserClient{config: trq.config}).Query()
...
// WithWorkflowNode tells the query-builder to eager-load the nodes that are connected to
// the "workflow_node" edge. The optional arguments are used to configure the query builder of the edge.
func (trq *TaskRecordQuery) WithWorkflowNode(opts ...func(*WorkflowNodeQuery)) *TaskRecordQuery {
	query := (&WorkflowNodeClient{config: trq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	trq.withWorkflowNode = query
	return trq
}
...
//		GroupBy(taskrecord.FieldStatus).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (trq *TaskRecordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TaskRecord, error) {
	var (
		...
		loadedTypes = [3]bool{
			trq.withUser != nil,
			trq.withTask != nil,
			trq.withWorkflowNode != nil,
		}
	)
	if trq.withUser != nil || trq.withTask != nil || trq.withWorkflowNode != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := trq.withUser; query != nil {
		if err := trq.loadUser(ctx, query, nodes, nil,
			func(n *TaskRecord, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := trq.withWorkflowNode; query != nil {
		if err := trq.loadWorkflowNode(ctx, query, nodes, nil,
			func(n *TaskRecord, e *WorkflowNode) { n.Edges.WorkflowNode = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (trq *TaskRecordQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*TaskRecord, init func(*TaskRecord), assign func(*TaskRecord, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*TaskRecord)
	for i := range nodes {
		if nodes[i].user_task_records == nil {
			continue
		}
		fk := *nodes[i].user_task_records
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_task_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (trq *TaskRecordQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*TaskRecord, init func(*TaskRecord), assign func(*TaskRecord, *Task)) error {
	...
	nodeids := make(map[int][]*TaskRecord)
	for i := range nodes {
		if nodes[i].task_task_records == nil {
			continue
		}
		fk := *nodes[i].task_task_records
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_task_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (trq *TaskRecordQuery) loadWorkflowNode(ctx context.Context, query *WorkflowNodeQuery, nodes []*TaskRecord, init func(*TaskRecord), assign func(*TaskRecord, *WorkflowNode)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*TaskRecord)
	for i := range nodes {
		if nodes[i].workflow_node_task_records == nil {
			continue
		}
		fk := *nodes[i].workflow_node_task_records
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "workflow_node_task_records" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
