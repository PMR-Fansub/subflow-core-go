...
func (uq *UserQuery) QueryTaskRecords() *TaskRecordQuery {
	query := (&TaskRecordClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTaskRecords: uq.withTaskRecords.Clone(),
...
func (uq *UserQuery) WithTaskRecords(opts ...func(*TaskRecordQuery)) *UserQuery {
	query := (&TaskRecordClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withTaskRecords = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withTaskRecords; query != nil {
		if err := uq.loadTaskRecords(ctx, query, nodes,
			func(n *User) { n.Edges.TaskRecords = []*TaskRecord{} },
			func(n *User, e *TaskRecord) { n.Edges.TaskRecords = append(n.Edges.TaskRecords, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadTaskRecords(ctx context.Context, query *TaskRecordQuery, nodes []*User, init func(*User), assign func(*User, *TaskRecord)) error {
	...
	query.Where(predicate.TaskRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TaskRecordsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_task_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_task_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_task_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
